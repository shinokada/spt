...
fn_clean() {
    ...
    # Safety guard against dangerous paths
    local safe_dir="${DEBTEMP_DIR%/}"
    if [ -z "$safe_dir" ] || [ "$safe_dir" = "/" ] || [ "$safe_dir" = "." ]; then
        echo "Error: Refusing to clean unsafe directory: '$DEBTEMP_DIR'"
        exit 1
    fi

    # Remove cache directory
    echo "Cleaning cache ..."
    if rm -rf "$safe_dir"; then
